...
\State \textbf{if} $\textit{step} > (V_{\max}/3.6\sigma)\,dt\cdot 1.5$ \textbf{then} drop sample, keep $W$ \Comment{jump guard}
...
Outbound) that filter the displayed rows by the LineZone-derived direction; the
...
\begin{itemize}
  ...
  \item \emph{Backend infrastructure}
...
\begin{itemize}
  ...
  deck.gl FOV polygon strategies (manual ring / curved corridor / trapezoid),
