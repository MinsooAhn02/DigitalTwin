...
camera the system streams HLS video, detects vehicles with Ultralytics YOLO26, an NMS-free end-to-end detector
(with YOLOv8 retained as a legacy fallback), tracks them
...
them with three complementary mechanisms: (i) a two-stage curved transform
...
scale factor that absorbs the residual geometric scale error and
is updated online via an ITS-referenced calibration loop. To give operators a persistent, city-scale view of road conditions beyond any
single active camera, the system additionally supports multi-camera background
monitoring, camera-level congestion clustering, and a SQLite time-series history
with charting. We describe the architecture and the key algorithms, and report
latency, throughput, tracking-stability, and speed metrics produced by a
measurement harness over live streams.
\end{abstract}
...
Table~\ref{tab:sota} summarises how this work compares with representative
traffic-monitoring approaches across the dimensions most relevant to uncalibrated
...
Dubska et al.~\cite{dubska2014} & \checkmark & \checkmark & \checkmark & $\times$ & \checkmark & \checkmark & $\times$ & $\times$ \\
...
samples accumulate. ROI edits trigger an immediate GPS ring recomputation and
...
pixel to local metres via $H_m$; stage two decomposes the displacement from the
snap point into along-road and lateral components, advances that arc length along
the centreline, interpolates the on-road position, and re-applies the lateral
offset perpendicular to the \emph{local} road bearing---tracking real road
curvature rather than a fixed tangent plane.
...
%=============================================================================
This work distinguishes itself from related systems along four axes.

\textbf{Versus flat-homography localization.} Existing CCTV-based vehicle trackers
apply a single perspective homography that treats the road as a flat plane; on
curved roads this displaces markers by several metres.
We introduce a two-stage transform that advances arc-length along the true road
centreline extracted from the national NodeLink dataset, keeping positions
geometrically correct on bends without any additional calibration target.

\textbf{Versus ITS dashboards.} National ITS portals (e.g., Korea's ITS OpenAPI)
publish CCTV streams and segment speeds as separate, human-facing views with no
per-vehicle analytics.
We \emph{fuse} the ITS segment speed as a machine-readable calibration target in
a closed online loop that converges a per-camera scale factor without any
ground-truth instrumentation on vehicles or road.

\textbf{Versus dedicated sensor systems.} Loop detectors, radar units, and
on-vehicle GPS loggers require roadside installation or vehicle participation.
Our system re-purposes public CCTV cameras that road authorities already operate,
adding no hardware to the road or to vehicles, yet tracking multiple vehicle
classes simultaneously across multiple views.

\textbf{Versus open-source YOLO/BoxMOT pipelines.} Generic multi-object tracking
pipelines produce bounding boxes and track IDs but lack geographic anchoring,
calibrated speed estimation, or support for uncalibrated cameras.
To our knowledge this is the first publicly available end-to-end implementation
(\url{https://github.com/MinsooAhn02/DigitalTwin}) that integrates NMS-free
YOLO26 detection with TensorRT~FP16 acceleration, road-centreline-anchored
localization, a five-stage ITS-calibrated speed estimator, and interactive
deck.gl map visualization---all operating on uncalibrated public infrastructure
cameras with no additional hardware.

The two authors co-designed the overall architecture and integrated their parts;
the split below reflects primary ownership.
...
\begin{itemize}
  ...
  the baseline ran YOLO frame-by-frame with no identity tracking.
  Added multi-object tracking via BoxMOT~\cite{boxmot} and a VRAM-based tier
  system that automatically selects ByteTrack~\cite{bytetrack},
  OC-SORT~\cite{ocsort}, BoT-SORT~\cite{botsort}, or DeepOC-SORT by available
  GPU memory, scaling from a CPU-only machine to a high-end workstation without
  code changes.
  \item \emph{TensorRT inference backend}:
  the original inference path was PyTorch-only. Added TensorRT~\cite{tensorrt}
  FP16 engine export and an auto-selection chain
  (engine $\to$ ONNX Runtime $\to$ PyTorch), raising throughput and reducing
  latency variance.
  \item \emph{YOLO26 upgrade}:
  migrated the default detector from YOLOv8~\cite{yolov8} to
  YOLO26~\cite{yolo26}, an NMS-free end-to-end architecture, combined with
  TensorRT to form the current production pipeline. Raised \texttt{YOLO\_CONF}
  from~0.25 to~0.30 since post-NMS filtering is no longer available at the
  detection level.
  \item \emph{CCTV-based five-stage speed estimator}:
  replaced the naive distance/time speed approximation with the filter of
  Algorithm~\ref{alg:speed} (physics jump guard, sliding-window OLS regression,
  EMA smoothing with spike rejection, PTS-derived time axis), addressing the
  specific challenges of CCTV-based measurement.
  Added a per-vehicle \texttt{speed\_reliable} flag: vehicles whose GPS position
  lies more than 100\,m from the camera snap point are excluded from the frame
  average and ITS calibration samples (far-field pixel noise amplification), and
  \texttt{is\_speeding} is suppressed for unreliable vehicles.
  \item \emph{Dwell counting --- bottleneck and parked-vehicle detection}:
  added dwell-frame counting and pixel-position memory to distinguish
  stopped-in-traffic from genuinely parked vehicles, surviving tracker ID
  cycling.
  \item \emph{Backend infrastructure and thread-safety}:
  built a FastAPI dual-path server with shared-detector coordination, congestion
  clustering, SQLite history store, and background multi-camera monitoring. To
  prevent data races across the dual-path inference pipeline, introduced a
  four-lock concurrency model (\texttt{\_state\_lock},
  \texttt{\_json\_file\_lock}, \texttt{roi\_manager.\_write\_lock},
  \texttt{\_frame\_count\_lock}) with all GPU-inference lock acquisitions
  routed through \texttt{asyncio.to\_thread}.
  Hardened HLS stream connection in \texttt{open\_video\_source}: after
  \texttt{isOpened()} succeeds, the function loops \texttt{read()} for up to
  3\,seconds to confirm a frame actually arrives---catching token-expired or
  404-segment failures that previously caused silent infinite-retry loops.
  \texttt{VideoStream.reconnect()} was moved to \texttt{asyncio.to\_thread} so
  the 3-second blocking validation no longer stalls the event loop.
  \item \emph{Movement-vector direction classification}:
  ...
  bearing axis with an EMA deadzone filter; LineZone crossing is retained as a
  fallback when road bearing is unavailable.
  \item \emph{GPS position post-processing pipeline}:
  added per-track EMA position smoothing to remove lateral jitter, and a
  lane-offset pass that laterally separates inbound and outbound vehicles by the
  local road bearing perpendicular, producing physically correct lane placement
  for Korean right-hand traffic.
  \item \emph{FOV polygon stabilization and ROI broadcast}:
  introduced a slow EMA filter on auto-calibrated FOV parameters (near/far
  distance, road width) to prevent polygon flicker; ROI edits now trigger an
  immediate GPS ring recomputation and live broadcast.
\end{itemize}

\subsection*{Dahyun Kwon --- Localization, Calibration, and Map Visualization}

\begin{itemize}
  \item \emph{In/Out vehicle counting baseline}:
  the original system had no reliable counting or direction assignment.
  ...
  \item \emph{Auto-calibration pipeline}:
  evolved calibration from a basic trapezoid heuristic to a full road-model pose
  solver---a four-parameter pinhole fit ($H$, pitch, yaw, lateral offset~$x_0$)
  via \texttt{scipy.optimize.least\_squares} (soft-L1) that persists per camera
  and refines across sessions. A cold-start fallback chain covers edge cases
  (saved prior $\to$ vehicle-bbox pose $\to$ GPS-grid).
  \item \emph{Per-camera scale correction loop}:
  designed and implemented an online ITS-referenced update loop that compares the
  measured rolling average against the official ITS segment speed and accumulates
  a per-camera \texttt{speed\_scale}, with a CV guard to skip volatile traffic,
  an adaptive $\alpha$ schedule (0.15 initially $\to$ 0.01 steady-state) for fast
  initial convergence, and a $\pm10\,\%$ per-update clamp to guard against corrupt
  ITS samples. The factor persists across sessions and accumulates toward an
  ITS-validated correction over extended operation.
  \item \emph{NodeLink road network integration}:
  identified the MOCT NodeLink dataset~\cite{moct_nodelink} as the geometric
  anchor and drove its full integration: road snapping, centreline extraction,
  bidirectional carriageway fix, and road-width derivation from lane count.
  \item \emph{Two-stage curved pixel$\to$GPS transform}:
  the flat homography places vehicles incorrectly on curved roads. Added
  arc-length interpolation along the road centreline that keeps GPS positions
  correct on curves (Algorithm~\ref{alg:curved}).
  ...
  \item \emph{Camera geometry utilization}:
  ...
  added deck.gl FOV polygon strategies (ROI GPS ring / manual calibration ring /
  straight-sided rectangle / default trapezoid), calibration overlay with
  NodeLink node snapping, and the calibration tab in the CCTV player.
\end{itemize}
