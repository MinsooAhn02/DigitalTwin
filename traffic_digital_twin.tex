...
(iii) geometric recovery of the absolute speed scale via a \emph{focal-free} pose solve
that reads the focal length from the dashed lane-marking period. The homography is
produced without manual setup by a warm-up pipeline that builds a vehicle-free
\emph{clean plate} to expose those markings and solve camera pose. An ITS-referenced
segment-speed cross-check is available as an optional refinement but is disabled by
default. The system additionally supports multi-camera background monitoring,
...
\begin{itemize}
  ...
  \item \textbf{Geometric speed-scale recovery.} The absolute longitudinal (speed) scale is
  recovered \emph{geometrically} by a focal-free pose solve that reads the focal length from
  the dashed lane-marking period on the warm-up clean plate, removing the dependence on any
  external speed reference. An ITS-referenced \texttt{speed\_scale} cross-check remains
  available as an \emph{optional, off-by-default} refinement for cameras where dashed
  markings are not observable.
...
any geometric scale error unchecked. Our pipeline can \emph{optionally} fuse the segment
speed back as a soft cross-check, though by default it recovers the longitudinal scale
geometrically (focal-free solve) and uses the ITS segment speed only as an independent
comparison.
...
per-axis novelty (curved localization, focal-free geometric scale recovery) is what we
claim as new.
...
\subsection{Optional ITS Scale Cross-Reference}
The focal-free pose solve (Section~\ref{sec:impl}-B) recovers the longitudinal scale
geometrically from the dashed-marking period, so it is the primary mechanism for absolute
speed scale; a residual error can remain only on cameras where dashed markings are not
observable. As an \emph{optional} refinement---\textbf{disabled by default}
(\texttt{ITS\_DRIVES\_SCALE=false})---the system can maintain a per-camera
\texttt{speed\_scale} factor that multiplies the geometric speed estimate
(Algorithm~\ref{alg:speed}, line~6), updated online using the ITS official segment speed as
a soft reference. With the option off (the default and evaluated configuration), the ITS
segment speed is shown only as an independent comparison (Section~\ref{sec:impl}-F) and is
never fed back into the estimate. When enabled, the ITS signal carries inherent
...
\textit{Run conditions.} Model \texttt{YOLO26m}, source: National Route\,3 Seongnam
Daewon\,IC (live ITS HLS, dense daytime traffic), measured for ${\approx}60$\,s per
configuration (20-frame warmup excluded), so the frame count per run varies with
throughput. Backend and tracker are varied across runs as shown in
Tables~\ref{tab:latency} and~\ref{tab:tracker}; all other conditions are held constant.
Each table is an independent measurement run over a different 60\,s window, so the shared
TensorRT\,+\,OC-SORT operating point differs slightly across
Tables~\ref{tab:latency}--\ref{tab:tracker}, and the tracking-stability counts vary with
the traffic present at each capture time. The latency/tracker runs use the uncalibrated
grid homography (timing is independent of calibration quality); the tracking-stability,
speed, and detection figures in Table~\ref{tab:track} are from a separate, longer
\emph{pose-calibrated} live run on the same camera (6{,}877 frames), included to confirm
the pipeline produces plausible output, not as ground-truth benchmarks. The FPS columns
report the offline harness's sequential single-stream throughput, which is lower than the
deployed pipeline's effective rate under dense traffic.

\begin{table}[t]
\caption{Inference backend comparison (OC-SORT tracker fixed, ${\approx}60$\,s per backend,
RTX\,4070 Laptop, National Route\,3 Seongnam Daewon\,IC, dense daytime traffic). Track stage
includes YOLO\,+\,BoxMOT end-to-end.}
\label{tab:latency}
\centering
\small
\begin{tabular}{@{}lrrr@{}}
...
\midrule
TensorRT FP16  & 16.3 & 19.9 & 59.6 \\
PyTorch (CUDA) & 28.0 & 33.1 & 35.1 \\
ONNX Runtime   & 29.3 & 38.1 & 33.4 \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[t]
\caption{Tracker comparison (TensorRT FP16 backend fixed, ${\approx}60$\,s each,
National Route\,3 Seongnam Daewon\,IC, dense daytime traffic).
BoT-SORT overhead is from OSNet appearance ReID; ByteTrack fragments heavily without
re-identification.}
\label{tab:tracker}
\centering
\scriptsize
...
\midrule
ByteTrack & 43.9 & 61.2 & 22.4 & 946 & 2940 &  13.1 \\
OC-SORT   & 16.3 & 19.9 & 59.6 & 272 & 1609 & 121.1 \\
BoT-SORT  & 60.7 & 78.6 & 16.3 &  79 &  214 & 176.0 \\
\bottomrule
\multicolumn{7}{@{}l}{\scriptsize IDs = unique track IDs; ID ev. = ID creation events; Life = mean lifetime.}\\
\end{tabular}
\end{table}

TensorRT FP16 achieves the lowest track-stage latency (16.3\,ms mean), 1.8$\times$ faster
than ONNX Runtime (29.3\,ms) and 1.7$\times$ faster than PyTorch (28.0\,ms) on the same GPU.
The harness additionally breaks end-to-end time into pipeline stages; the homography
transform and analytics stages each contribute under 0.5\,ms (median 0.06\,ms and 0.43\,ms
respectively), confirming that the localization and speed-estimation layers add negligible
overhead to the dominant tracking stage. With the backend fixed to TensorRT, the dense
daytime scene separates the trackers sharply. OC-SORT is simultaneously the fastest
(16.3\,ms) and far more stable than ByteTrack, whose lack of re-identification fragments the
heavy traffic into 946 short-lived tracks (2{,}940 ID-creation events, 13-frame mean
lifetime) and inflates association cost to 43.9\,ms. BoT-SORT's OSNet appearance ReID yields
the most stable tracks (79 IDs, 214 ID events, 176-frame mean lifetime) but is the slowest
(60.7\,ms mean, 78.6\,ms p95) due to the embedding network. OC-SORT therefore gives the best
stability/latency trade-off for this workload, while BoT-SORT is preferable when maximum
ID persistence matters and the GPU budget allows.
...
\textbf{Optional ITS scale cross-reference (archived).}
The default pipeline recovers the speed scale geometrically (the focal-free solve), so the
ITS \texttt{speed\_scale} loop is an optional, off-by-default refinement rather than a core
mechanism. For completeness we report an archived snapshot from earlier operation with the
loop enabled: Table~\ref{tab:scale} summarises the factors accumulated across the cameras
monitored in that mode. The distribution is informative as a diagnostic---it shows how far
the raw geometric projection deviated per camera before any focal-free correction: the bulk
cluster within $\pm$20\,\% of 1.0 (geometric calibration roughly correct), while the tails
reflect steep-pitch / far-field cameras that compress apparent displacement (below~0.8) and
wide-angle / low-mounted cameras that exaggerate it (above~1.2). It is \emph{not} part of
the default evaluated configuration.

\begin{table}[t]
\caption{Distribution of \texttt{speed\_scale} factors from an \emph{archived} run with the
optional ITS loop enabled (off by default in the current pipeline, which recovers scale
geometrically via the focal-free solve). Values below~1.0 indicate geometric
underestimation of displacement (camera axis oblique to the road); above~1.0,
overestimation. Shown as a per-camera projection-bias diagnostic, not a default-mode result.}
\label{tab:scale}
\centering
\small
\begin{tabular}{@{}lrr@{}}
...
scale error the homography carries. Longitudinal scale is recovered \emph{geometrically} by
the focal-free pose solve, which reads the focal length from the dashed-marking period when
those markings are observable on the clean plate. For cameras where they are not, the
residual scale error can \emph{optionally} be absorbed by the ITS-referenced
\texttt{speed\_scale} loop (disabled by default), which nudges a per-camera multiplicative
factor toward the official segment speed.
The mechanisms are therefore complementary rather than redundant: the centreline geometry
fixes \emph{where on the road} a vehicle is placed, and the focal-free solve fixes
\emph{how far} the geometry believes it has travelled, with the optional ITS loop available
only as a fallback cross-check.
...
When enabled, the optional ITS loop uses an adaptive learning rate (0.15 on the first two updates, decaying to 0.01 thereafter, polled every 5\,min) and treats the ITS segment speed as an imprecise soft reference, since ITS figures reflect 1-km road-segment averages that may not align with the camera's exact observation zone or direction. It is \emph{off by default}: the focal-free geometric solve is the primary scale mechanism, so the pipeline does not depend on ITS convergence. When the loop is used, the accumulated factors should be read as a per-camera projection-bias diagnostic rather than a definitive calibration.
...
estimator, and geometric speed-scale recovery via a focal-free pose solve (with an
optional, off-by-default ITS segment-speed cross-check), wrapped in a resilient dual-path
pipeline with congestion analytics and a historical store. The automatic calibration
pipeline---a warm-up clean-plate pose solver that recovers focal length and scale
geometrically---substantially reduces the per-camera manual setup required to deploy across
many cameras, and the system needs no external speed reference to operate.
