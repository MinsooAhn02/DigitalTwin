...
loop. The system additionally supports multi-camera background monitoring,
...
pinhole camera at $10$\,m height, $18^\circ$ downward pitch, focal length $f=864$\,px
($1.2\times$ a $720$\,px frame height), snap point $10$\,m ahead of the camera;
...
in the automatic calibration path ($f_y = 1.2h$, a $\pm$15--20\,\% FoV-dependent
scale error), and it falls almost entirely in the \emph{longitudinal} (depth)
...
orthogonality claim and the role assignment (geometry for lateral placement, the
\texttt{speed\_scale} loop for longitudinal scale), while showing that the curved
...
scale error the homography carries.
That residual scale error is absorbed instead by the ITS-referenced
\texttt{speed\_scale} loop, which converges a per-camera multiplicative factor
against the official segment speed.
The two mechanisms are therefore complementary rather than redundant: the geometry
fixes \emph{where on the road} a vehicle is placed, and the ITS loop fixes
\emph{how far} the geometry believes it has travelled.
