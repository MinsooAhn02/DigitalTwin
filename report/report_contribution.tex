...
\textit{Run conditions.} Model \texttt{YOLO26m}, backend \texttt{TensorRT}
(auto-selected), tracker \texttt{BotSort} (medium tier, $\ge$6\,GB VRAM),
source: Goyang Jugyo Underpass, National Rd.\,39 (live ITS HLS),
4{,}559 frames (935.9\,s). Speed and vehicle-count observations in
Tables~\ref{tab:track} are single-session figures that reflect the specific
camera, time of day, and traffic conditions at the time of measurement; they
are included to confirm the pipeline produces plausible output, not as
ground-truth benchmarks.

\begin{table}[t]
\caption{Per-stage latency (ms) and throughput (4{,}559-frame eval run,
RTX\,4070 Laptop + TensorRT\,10.16 + BotSort). Figures are hardware-specific.}
\label{tab:latency}
\centering
\small
\begin{tabular}{@{}lccc@{}}
...
Detections -- bus             & 277 \\
\bottomrule
\multicolumn{2}{@{}p{\linewidth}}{\footnotesize $^{\dagger}$Far-field outliers; median 75.1\,km/h is representative.} \\
\multicolumn{2}{@{}p{\linewidth}}{\footnotesize $^{\ddagger}$Speed requires $\ge W$ frames of track history; single-frame detections excluded.} \\
\end{tabular}
\end{table}
